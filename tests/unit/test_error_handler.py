...
class TestCustomExceptions:
    ...
    def test_chat_not_found_exception(self):
        ...
        exc = ChatNotFoundException("Chat not found", chat_id="123")

        assert exc.message == "Chat not found"
        assert exc.chat_id == "123"
        assert exc.error_type == "CHAT_NOT_FOUND"
        assert exc.status_code == 404

    def test_channel_private_exception(self):
        """Test ChannelPrivateException."""
        exc = ChannelPrivateException("Channel is private")

        assert exc.message == "Channel is private"
        assert exc.error_type == "CHANNEL_PRIVATE"
...
class TestErrorTracker:
    ...
    def test_track_error(self, error_tracker):
        ...
        assert tracked_error["error_message"] == "Test error"
    ...
    def test_error_limit(self, error_tracker):
        ...
        assert error_tracker.errors[0]["context"]["id"] == 4
        assert error_tracker.errors[-1]["context"]["id"] == 2
    ...
    def test_get_recent_errors(self, error_tracker):
        ...
        assert len(recent) == 3
        assert recent[0]["context"]["id"] == 4  # Most recent
        assert recent[-1]["context"]["id"] == 2  # Third most recent
...
class TestErrorHandlerIntegration:
    ...
    def test_error_tracker_with_metrics(self, error_tracker):
        ...
        with patch('src.telegram_toolkit_mcp.core.error_handler.get_metrics_collector') as mock_get_metrics:
            mock_metrics = Mock()
            ...
            mock_metrics.record_error.assert_called_once_with(
                "FloodWaitException", "error_handler"
            )
...
class TestErrorHandlerEdgeCases:
    ...
    def test_exception_with_no_message(self):
        ...
        assert isinstance(mapped, TelegramMCPException)
        assert "NoMessageException" in mapped.message
    ...
    def test_flood_wait_exception_with_zero_retry(self):
        ...
        exc = FloodWaitException("Rate limit", retry_after=0)

        assert exc.retry_after == 0
        error_dict = exc.to_dict()
        assert "0 seconds" in error_dict["detail"]
...
class TestErrorHandlerConfiguration:
    ...
    def test_error_tracker_configuration(self):
        ...
        original_max = tracker.max_errors

        # Test with different limits
        tracker.max_errors = 10
        assert tracker.max_errors == 10

        # Reset
        tracker.max_errors = original_max
    ...
    def test_exception_to_dict_completeness(self):
        ...
        exceptions = [
            ...
            TelegramMCPException("Generic error")
        ]
